...
# Create all Cloudflare resources (D1, KV, R2, Pages project)
# This is idempotent - safe to run multiple times
bun run setup

# The setup script will:
# 1. Create D1 database and extract ID
# 2. Create KV namespace and extract ID
# 3. Create R2 bucket
# 4. Create Cloudflare Pages project
# 5. Generate wrangler.toml with actual resource IDs
# 6. Run migrations
# 7. Seed POC user (mahesh.local)
```

**Note:** The `wrangler.toml` file is generated automatically by the setup script. It contains your actual resource IDs and is not tracked in git. See `wrangler.toml.example` for reference.
...
Located in `wrangler.toml` (auto-generated by `bun run setup`).

See `wrangler.toml.example` for the template. Two environments available:
...
All database setup is handled by the setup script:

```bash
# Run once to create database and run migrations
bun run setup
```

Manual commands (if needed):
```bash
# Create D1 database only
bun run db:create
...
bun run teardown         # Destroy all Cloudflare resources (interactive)
...
# Database (manual, use setup script instead)
...
1. `bun run setup` - First time only (creates all resources)
2. `bun run dev` - For development
3. `bun run deploy` - For production (after `bun run build:all`)
4. `bun run teardown` - If you need to clean up resources
