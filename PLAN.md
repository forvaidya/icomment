...
- [ ] **BadTameez Branding Setup:**
  - [ ] Create `public/favicon.ico` (skull icon, 32x32 or 16x16)
  - [ ] Create `public/logo.svg` (BadTameez skull logo)
  - [ ] Create `public/index.html` (static HTML shell with favicon link)
  - [ ] Set APP_LOGO_URL to `/logo.svg` in wrangler.toml
  - [ ] Set APP_NAME to `BadTameez` (prod) and `BadTameez (Dev)` (dev)
  - [ ] Set BRAND_COLOR to `#ff0000` (red, can be customized)
...
│   ├── favicon.ico                     # Favicon (skull icon)
│   ├── logo.svg                        # BadTameez skull logo (SVG)
...
- `APP_NAME` - Application name displayed in UI (default: "BadTameez", dev: "BadTameez (Dev)")
- `APP_LOGO_URL` - URL to app logo/branding SVG (default: `/logo.svg` - BadTameez skull logo)
- `BRAND_COLOR` - Primary brand color in hex format (default: "#ff0000" - red)

**BadTameez Branding:**
- **Name**: "BadTameez" (meaning "arrogant/disrespectful")
- **Philosophy**: All kinds of comments welcome, no censorship
- **Logo**: Skull icon (SVG in `/public/logo.svg`)
- **Favicon**: Skull icon (ICO in `/public/favicon.ico`)
- **Color**: Red (#ff0000) by default, customizable
